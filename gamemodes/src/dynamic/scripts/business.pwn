...
CMD:editarempresa(playerid, params[])//Game Admin + Property Team
{
	...
	if (!strcmp(type, "localizacao", true))
	{
		...
		Business_Save(id);
       	...
       	Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s ajustou a localização da empresa: %i.", AccountData[playerid][e_ACCOUNT_NAME], id);
	}
	else if (!strcmp(type, "interior", true))
	{
		...
		Business_Save(id);
		Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s ajustou o interior da empresa: %i.", AccountData[playerid][e_ACCOUNT_NAME], id);
	}
	else if (!strcmp(type, "preco", true))
	{
		...
		Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s ajustou o preço da empresa: %i de %s para %s.", AccountData[playerid][e_ACCOUNT_NAME], id, FormatMoney(BusinessData[id][e_BUSINESS_PRICE]), FormatMoney(price));
	...
	else if (!strcmp(type, "estoque", true))
	{
		...
		Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s ajustou o estoque da empresa: %i para %i produtos.", AccountData[playerid][e_ACCOUNT_NAME], id, amount);
	}
	else if (!strcmp(type, "nome", true))
	{
		...
		Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s ajustou o nome da empresa: %i para \"%s\".", AccountData[playerid][e_ACCOUNT_NAME], id, name);
	}
	else if (!strcmp(type, "tipo", true))
	{
	    ...
	    Business_Save(id);
	  	...
	  	Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s ajustou o tipo da empresa: %i para %i.", AccountData[playerid][e_ACCOUNT_NAME], id, typeint);
	}
	return 1;
}
