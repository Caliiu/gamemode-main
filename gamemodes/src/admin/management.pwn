...
Dialog:DIALOG_MANAGE_HOME(playerid, response, listitem, inputtext[])
{
	...
	else if (!strcmp(inputtext, "Status", true))
	{
		...
		Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s tornou a Closed Beta %s no servidor.", AccountData[playerid][e_ACCOUNT_NAME], (GetGVarInt("CLOSED_BETA") ? ("ativa") : ("desativada")));
