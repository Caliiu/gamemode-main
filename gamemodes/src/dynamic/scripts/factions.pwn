...
Faction_Load()
{
	...
	inline Faction_OnLoaded()
	{
		...
		for (new i = 0; i < MAX_FACTIONS; i++)
		{
			...
			{
				format(colStr, sizeof(colStr), "Skin%d", j + 1); 
				cache_get_value_name_int(i, colStr, FactionData[i][e_FACTION_SKINS][j]);
			}

			for (new j = 0; j < MAX_FACTION_WEAPONS; j++)
			{
				format(colStr, sizeof(colStr), "Weapon%d", j + 1);
				cache_get_value_name_int(i, colStr, FactionData[i][e_FACTION_WEAPONS][j]);

				format(colStr, sizeof(colStr), "Ammo%d", j + 1);
				cache_get_value_name_int(i, colStr, FactionData[i][e_FACTION_AMMO][j]);
			}
...
Faction_Save(id)
{
    ...
    mysql_format(MYSQL_CUR_HANDLE, query, sizeof(query), 
    ...
    {
        format(buffer, sizeof(buffer), ", `Skin%d` = %d", i + 1, FactionData[id][e_FACTION_SKINS][i]);
        strcat(query, buffer);
    }

    for(new i = 0; i < MAX_FACTION_WEAPONS; i++)
    {
        format(buffer, sizeof(buffer), ", `Weapon%d` = %d, `Ammo%d` = %d", i + 1, FactionData[id][e_FACTION_WEAPONS][i], i + 1, FactionData[id][e_FACTION_AMMO][i]);
        strcat(query, buffer);
    }

    format(buffer, sizeof(buffer), " WHERE `ID` = %d LIMIT 1;", FactionData[id][e_FACTION_ID]);
    strcat(query, buffer);

    inline Faction_OnSaved() {}
    MySQL_TQueryInline(MYSQL_CUR_HANDLE, using inline Faction_OnSaved, query);
...
Faction_RefreshArsenal(id, index)
{
	...
	FactionArsenalData[id][index][e_FACTION_ARSENAL_LABEL] = CreateDynamic3DTextLabel(
		"{747474}[Arsenal]\n{FFFFFF}/arsenal para acessar esse arsenal.", 
		-1, 
...
Faction_NearestArsenal(playerid, Float:radius=2.5)
{
	...
	for (new i = 0; i < MAX_FACTION_ARSENAL; i++)
	{
		...
		if (GetPlayerVirtualWorld(playerid) != FactionArsenalData[id][i][e_FACTION_ARSENAL_WORLD]) 
			continue;
...
Faction_LoadLocker(id)
{
    ...
    inline Faction_OnLoadedLockers()
    {
        ...
        for (new i = 0; i < MAX_FACTION_LOCKER; i++)
        {
            if (i >= rows)
            {
                FactionLockerData[id][i][e_FACTION_LOCKER_EXISTS] = false;
                continue;
            }
            ...
            for(new j = 0; j < MAX_LOCKER_SLOTS; j++)
            {
                format(colStr, sizeof(colStr), "Item%d", j + 1);
                cache_get_value_name_int(i, colStr, FactionLockerData[id][i][e_FACTION_LOCKER_ITEMS][j]);

                format(colStr, sizeof(colStr), "Amount%d", j + 1);
                cache_get_value_name_int(i, colStr, FactionLockerData[id][i][e_FACTION_LOCKER_AMOUNT][j]);
            }
...
Faction_CreateLocker(playerid, id)
{
	...
	if (index != -1)
	{
		...
		// MySQL_TQueryInline(MYSQL_CUR_HANDLE, using inline Faction_OnCreatedLocker, "INSERT IGNORE INTO `FacçãoArmario` (`Facção`) VALUES (%i);", FactionData[id][e_FACTION_ID]);
		MySQL_TQueryInline(MYSQL_CUR_HANDLE, using inline Faction_OnCreatedLocker,
		"INSERT INTO `FacçãoArmário` (`Facção`, `PosX`, `PosY`, `PosZ`, `World`, `Interior`) \
		VALUES (%i, '%f', '%f', '%f', %i, %i);",
		FactionData[id][e_FACTION_ID],
...
Faction_RefreshLocker(id, index)
{
	...
	FactionLockerData[id][index][e_FACTION_LOCKER_LABEL] = CreateDynamic3DTextLabel(
		"{747474}[Armário]\n{FFFFFF}/armario para acessar esse armário.", 
		-1, 
		FactionLockerData[id][index][e_FACTION_LOCKER_POS][0],
		FactionLockerData[id][index][e_FACTION_LOCKER_POS][1],
		FactionLockerData[id][index][e_FACTION_LOCKER_POS][2],
...
Faction_SaveLocker(id, index)
{
    ...
    for(new j = 0; j < MAX_LOCKER_SLOTS; j++)
    {
        format(buffer, sizeof(buffer), ", `Item%d`=%d, `Amount%d`=%d", 
            j + 1, 
            FactionLockerData[id][index][e_FACTION_LOCKER_ITEMS][j],
            j + 1, 
            FactionLockerData[id][index][e_FACTION_LOCKER_AMOUNT][j]
        );
        strcat(query, buffer); 
    }

    format(buffer, sizeof(buffer), " WHERE `ID` = %i AND `Facção` = %i;", 
        FactionLockerData[id][index][e_FACTION_LOCKER_ID], 
        FactionData[id][e_FACTION_ID]
    );
    strcat(query, buffer);

    inline Faction_OnSavedLocker() {}
    MySQL_TQueryInline(MYSQL_CUR_HANDLE, using inline Faction_OnSavedLocker, query);
...
Faction_NearestLocker(playerid, Float:radius=2.5)
{
    ...
    for (new i = 0; i < MAX_FACTION_LOCKER; i++)
    {
        ...
        if (GetPlayerInterior(playerid) != FactionLockerData[id][i][e_FACTION_LOCKER_INTERIOR])
            continue;

        if (GetPlayerVirtualWorld(playerid) != FactionLockerData[id][i][e_FACTION_LOCKER_WORLD])
            continue;
...
CMD:criarfaccao(playerid, params[])
{
	...
	Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s criou a facção %s (ID: %i).", AccountData[playerid][e_ACCOUNT_NAME], params, index);
...
CMD:editarfaccao(playerid, params[])
{
	...
	if (sscanf(params, "ds[24]S()[128]", id, type, string))
 	{
	    ...
	    SendClientMessage(playerid, COLOR_BEGE, "[Opções]: tierpontos, tieratualizacao, cofre, skin, arma");
		return 1;
	}
	...
	if (!strcmp(type, "nome", true))
	{
		...
		Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s ajustou o nome da facção %i para \"%s\".", AccountData[playerid][e_ACCOUNT_NAME], id, name);
	...
	else if (!strcmp(type, "abreviacao", true) || !strcmp(type, "abreviaçao", true) || !strcmp(type, "abreviacão", true) || !strcmp(type, "abreviação", true))
	{
		...
		Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s ajustou a abreviação da facção %i para \"%s\".", AccountData[playerid][e_ACCOUNT_NAME], id, abbrev);
	...
	else if (!strcmp(type, "tipo", true))
	{
		...
		Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s ajustou o tipo da facção %i para %s.", AccountData[playerid][e_ACCOUNT_NAME], id, g_arrayFactionTypes[tipo]);
	...
	else if (!strcmp(type, "cor", true))
	{
		...
		Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s ajustou o tipo da facção %i para \"%06x\".", AccountData[playerid][e_ACCOUNT_NAME], id, cor >>> 8);
	...
	else if (!strcmp(type, "cargopadrao", true) || !strcmp(type, "cargopadrão"))
	{
		...
		Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s ajustou o cargo padrão da facção %i para \"%s\".", AccountData[playerid][e_ACCOUNT_NAME], id, rank);
	...
	else if (!strcmp(type, "cofre", true))
	{
		...
		Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s ajustou o cofre da facção %i para %s.", AccountData[playerid][e_ACCOUNT_NAME], id, FormatMoney(value));
	...
	else if (!strcmp(type, "skin", true))
    {
        ...
        if (sscanf(string, "ii", slot, skinid))
            return SendUsageMessage(playerid, "/editarfaccao id skin [Slot 1-8] [ID Skin (0 para remover)]");

        if (!(1 <= slot <= MAX_FACTION_SKINS))
            return SendErrorMessage(playerid, "O slot deve ser entre 1 e 8.");

        if (skinid < 0 || skinid > 311) 
            return SendErrorMessage(playerid, "ID de skin inválido (Use 0 para limpar).");

        FactionData[id][e_FACTION_SKINS][slot - 1] = skinid;
        
        Faction_Save(id);

        Log_Create("[Faction Team] Facções editadas", "%s alterou a Skin Slot %d da facção %s para ID %d [%i]", AccountData[playerid][e_ACCOUNT_NAME], slot, FactionData[id][e_FACTION_ABBREV], skinid, FactionData[id][e_FACTION_ID]);
        Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s alterou a Skin (Slot %d) da facção %i para ID %d.", AccountData[playerid][e_ACCOUNT_NAME], slot, id, skinid);
        
        SendClientMessageEx(playerid, COLOR_GREEN, "Você definiu a Skin do Slot %d para o ID %d.", slot, skinid);
    }
    else if (!strcmp(type, "arma", true))
    {
        static 
			slot, 
			weaponid, 
        ...
        if (sscanf(string, "iii", slot, weaponid, ammo))
            return SendUsageMessage(playerid, "/editarfaccao id arma [Slot 1-10] [ID Arma (0 remove)] [Munição]");

        if (!(1 <= slot <= MAX_FACTION_WEAPONS))
            return SendErrorMessage(playerid, "O slot deve ser entre 1 e 10.");

        if (weaponid < 0 || weaponid > 46) 
            return SendErrorMessage(playerid, "ID de arma inválido.");

        FactionData[id][e_FACTION_WEAPONS][slot - 1] = weaponid;
        FactionData[id][e_FACTION_AMMO][slot - 1] = ammo;
        ...
        Log_Create("[Faction Team] Facções editadas", "%s alterou a Arma Slot %d da facção %s para %s (%d) [%i]", AccountData[playerid][e_ACCOUNT_NAME], slot, FactionData[id][e_FACTION_ABBREV], wName, ammo, FactionData[id][e_FACTION_ID]);
        Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s alterou a Arma (Slot %d) da facção %i para %s (%d munição).", AccountData[playerid][e_ACCOUNT_NAME], slot, id, wName, ammo);
        
        SendClientMessageEx(playerid, COLOR_GREEN, "Você definiu a Arma do Slot %d para %s (%d balas).", slot, wName, ammo);
    }
...
CMD:destruirfaccao(playerid, params[])
{
	...
	Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s deletou a facção \"%s\" (ID: %i).", AccountData[playerid][e_ACCOUNT_NAME], FactionData[id][e_FACTION_NAME], FactionData[id][e_FACTION_ID]);
...
CMD:setlider(playerid, params[])
{
	...
	Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s deu a liderança da facção \"%s\" para %s.", AccountData[playerid][e_ACCOUNT_NAME], FactionData[faction][e_FACTION_NAME], Character_GetName(id, false));
...
CMD:criararmario(playerid, params[])
{
	if (!Admin_CheckTeam(playerid, e_ADMIN_TEAM_FACTION))
		return true;
	...
	if (sscanf(params, "i", factionid))
		return SendUsageMessage(playerid, "/criararmario [id da facção]");

	if ((factionid < 0 || factionid >= MAX_FACTIONS) || !FactionData[factionid][e_FACTION_EXISTS])
		return SendErrorMessage(playerid, "Você especificou um ID de facção inválido.");

	id = Faction_CreateLocker(playerid, factionid);

	if (id == -1)
		return SendErrorMessage(playerid, "O servidor atingiu o limite de armários de facção.");

	SendClientMessageEx(playerid, COLOR_GREEN, "Você criou com sucesso o armário de facção ID: %i.", id);

	return true;
}

// Arsenal Comandos
CMD:criararsenal(playerid, params[])
{
	if (!Admin_CheckTeam(playerid, e_ADMIN_TEAM_FACTION))
		return true;
    ...
    if (sscanf(params, "i", factionid))
        return SendClientMessage(playerid, COLOR_BEGE, "USE: /criararsenal [ID da Facção]");

    if (!(0 <= factionid < MAX_FACTIONS) || !FactionData[factionid][e_FACTION_EXISTS])
        return SendErrorMessage(playerid, "A facção especificada não existe.");

    id = Faction_CreateArsenal(playerid, factionid);

    if (id == -1)
        return SendErrorMessage(playerid, "O servidor atingiu o limite de arsenais.");

    SendClientMessageEx(playerid, COLOR_GREEN, "Arsenal criado (Slot ID: %d) para a facção %s.", id, FactionData[factionid][e_FACTION_NAME]);
    Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s criou um arsenal para a facção %s.", AccountData[playerid][e_ACCOUNT_NAME], FactionData[factionid][e_FACTION_NAME]);

    return true;
}

CMD:destruirarsenal(playerid, params[])
{
	if (!Admin_CheckTeam(playerid, e_ADMIN_TEAM_FACTION))
		return true;
    ...
    if (sscanf(params, "ii", factionid, id))
        return SendClientMessage(playerid, COLOR_BEGE, "USE: /destruirarsenal [ID da Facção] [ID do Slot]");

    if (!(0 <= factionid < MAX_FACTIONS) || !FactionData[factionid][e_FACTION_EXISTS])
        return SendErrorMessage(playerid, "A facção especificada não existe.");

    if (!(0 <= id < MAX_FACTION_ARSENAL) || !FactionArsenalData[factionid][id][e_FACTION_ARSENAL_EXISTS])
        return SendErrorMessage(playerid, "Não existe um arsenal criado no Slot %d para esta facção.", id);

    if (Faction_DestroyArsenal(factionid, id))
    {
        SendClientMessageEx(playerid, COLOR_GREEN, "Arsenal (Slot %d) da facção %s foi excluído.", id, FactionData[factionid][e_FACTION_NAME]);
        Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s excluiu o arsenal slot %d da facção %s.", AccountData[playerid][e_ACCOUNT_NAME], id, FactionData[factionid][e_FACTION_NAME]);
    }
    else
    {
        SendErrorMessage(playerid, "Ocorreu um erro ao tentar excluir o arsenal.");
    }
