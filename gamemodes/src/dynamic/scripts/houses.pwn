...
CMD:editarcasa(playerid, params[])
{
	...
	if (!strcmp(type, "localizacao", true))
	{
		...
		House_Save(id);
      	...
      	Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s ajustou a localização da casa: %d.", AccountData[playerid][e_ACCOUNT_NAME], id);
	}
	else if (!strcmp(type, "interior", true))
	{
		...
		House_Save(id);
		Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s ajustou o interior da casa: %d.", AccountData[playerid][e_ACCOUNT_NAME], id);
	}
	else if (!strcmp(type, "preco", true))
	{
	    ...
	    House_Save(id);
       	...
       	Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s ajustou o preço da casa: %d para %s.", AccountData[playerid][e_ACCOUNT_NAME], id, FormatMoney(price));
	}
