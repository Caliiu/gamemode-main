...
Dialog:DIALOG_ENT_EDIT_ICON(playerid, response, listitem, inputtext[])
{
	if (response && (0 <= listitem < sizeof s_arrayEntranceIcons))
	{
		...
		Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s ajustou o ícone da entrada %i para o modelo %i.", AccountData[playerid][e_ACCOUNT_NAME], index, icon);
...
CMD:criarentrada(playerid, params[])
{
	...
	Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s criou a entrada \"%s\" (%i)", AccountData[playerid][e_ACCOUNT_NAME], params, index);
...
CMD:editarentrada(playerid, params[])
{
	...
	if (!strcmp(type, "nome", true))
	{
		...
		Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s ajustou o nome da entrada %i para \"%s\".", AccountData[playerid][e_ACCOUNT_NAME], id, name);
	...
	else if (!strcmp(type, "localizacao", true))
	{
		...
		Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s ajustou a posição da entrada \"%s\".", AccountData[playerid][e_ACCOUNT_NAME], EntranceData[id][e_ENTRANCE_NAME]);
	}
	else if (!strcmp(type, "interior", true))
	{
		...
		Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s ajustou a posição do interior da entrada \"%s\".", AccountData[playerid][e_ACCOUNT_NAME], EntranceData[id][e_ENTRANCE_NAME]);
	}
	else if (!strcmp(type, "mundo", true))
	{
		...
		Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s ajustou o mundo da entrada \"%s\".", AccountData[playerid][e_ACCOUNT_NAME], EntranceData[id][e_ENTRANCE_NAME]);
	}
...
CMD:destruirentrada(playerid, params[])
{
	...
	Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s deletou a entrada \"%s\" (%i)", AccountData[playerid][e_ACCOUNT_NAME], EntranceData[index][e_ENTRANCE_NAME], index);
