...
CMD:aa(playerid)
{
	...
	{
		SendClientMessage(playerid, COLOR_BEGE, "[Junior Admin] /aduty, /(a)dmin, /logs, /lerlog, /buscarlog, /ir, /trazer, /setfome, /setsede, /setsono, /refill");	
	}
...
CMD:aduty(playerid)
{
	...
	Admin_SendMessage(1, COLOR_YELLOW, "[PAINEL-ADMIN] %s está agora %s serviço administrativo.", AccountData[playerid][e_ACCOUNT_NAME], (AccountData[playerid][e_ACCOUNT_ADMIN_DUTY] ? ("em") : ("fora de")));
...
CMD:setfome(playerid, params[])
{
	...
	static target, hunger;

	if (sscanf(params, "ui", target, hunger))
		return SendUsageMessage(playerid, "/setfome [Nome/ParteDoNome/Id] [0-100]");

	if (hunger < 0 || hunger == 1000)
		return SendErrorMessage(playerid, "Use um valor de 0 a 1000.");

	if (!CheckTargetId(playerid, target, .adminCmd = true))
		return false;

	Log_Create("Admin CMD", "[SETFOME] %s usou no personagem %s (%s) - Fome: %i", AccountData[playerid][e_ACCOUNT_NAME], Character_GetName(target, false), AccountData[target][e_ACCOUNT_NAME], hunger);
	Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s setou a fome de %s para %i.", AccountData[playerid][e_ACCOUNT_NAME], Character_GetName(target, false), hunger);
	
	SendClientMessageEx(playerid, -1, "Você setou a fome de %s (%s) para %i.", Character_GetName(target, false), AccountData[target][e_ACCOUNT_NAME], hunger);
	SendClientMessageEx(target, -1, "Administrador %s alterou sua fome para %i.", AccountData[playerid][e_ACCOUNT_NAME], hunger);

	CharacterData[target][e_CHARACTER_HUNGER] = hunger;
...
CMD:setsede(playerid, params[])
{
	...
	static target, thrist;

	if (sscanf(params, "ui", target, thrist))
		return SendUsageMessage(playerid, "/setsede [Nome/ParteDoNome/Id] [0-100]");

	if (thrist < 0 || thrist == 1000)
		return SendErrorMessage(playerid, "Use um valor de 0 a 1000.");

	if (!CheckTargetId(playerid, target, .adminCmd = true))
		return false;

	Log_Create("Admin CMD", "[SETSEDE] %s usou no personagem %s (%s) - Sede: %i", AccountData[playerid][e_ACCOUNT_NAME], Character_GetName(target, false), AccountData[target][e_ACCOUNT_NAME], thrist);
	Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s setou a sede de %s para %i.", AccountData[playerid][e_ACCOUNT_NAME], Character_GetName(target, false), thrist);
	
	SendClientMessageEx(playerid, -1, "Você setou a sede de %s (%s) para %i.", Character_GetName(target, false), AccountData[target][e_ACCOUNT_NAME], thrist);
	SendClientMessageEx(target, -1, "Administrador %s alterou sua sede para %i.", AccountData[playerid][e_ACCOUNT_NAME], thrist);

	CharacterData[target][e_CHARACTER_THIRST] = thrist;
	Character_Save(target);

	return true;
}

CMD:setsono(playerid, params[])
{
	if (!Admin_CheckPermission(playerid, JUNIOR_ADMIN))
		return false;

	static target, esleep;

	if (sscanf(params, "ui", target, esleep))
		return SendUsageMessage(playerid, "/setsono [Nome/ParteDoNome/Id] [0-100]");

	if (esleep < 0 || esleep == 1000)
		return SendErrorMessage(playerid, "Use um valor de 0 a 1000.");

	if (!CheckTargetId(playerid, target, .adminCmd = true))
		return false;

	Log_Create("Admin CMD", "[SETSONO] %s usou no personagem %s (%s) - Sono: %i", AccountData[playerid][e_ACCOUNT_NAME], Character_GetName(target, false), AccountData[target][e_ACCOUNT_NAME], esleep);
	Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s setou o sono de %s para %i.", AccountData[playerid][e_ACCOUNT_NAME], Character_GetName(target, false), esleep);
	
	SendClientMessageEx(playerid, -1, "Você setou o sono de %s (%s) para %i.", Character_GetName(target, false), AccountData[target][e_ACCOUNT_NAME], esleep);
	SendClientMessageEx(target, -1, "Administrador %s alterou seu sono para %i.", AccountData[playerid][e_ACCOUNT_NAME], esleep);

	CharacterData[target][e_CHARACTER_SLEEP] = esleep;
	Character_Save(target);
...
CMD:refill(playerid, params[])
{
	...
	if (sscanf(params, "u", target))
		return SendUsageMessage(playerid, "/refill [Nome/ParteDoNome/Id]");

	if (!CheckTargetId(playerid, target, .adminCmd = true))
		return false;

	Log_Create("Admin CMD", "[REFILL] %s usou no personagem %s (%s)", AccountData[playerid][e_ACCOUNT_NAME], Character_GetName(target, false), AccountData[target][e_ACCOUNT_NAME]);
	Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s setou as necessidades de %s para 100.", AccountData[playerid][e_ACCOUNT_NAME], Character_GetName(target, false));
	
	SendClientMessageEx(playerid, -1, "Você setou as necessidades %s (%s) para 100.", Character_GetName(target, false), AccountData[target][e_ACCOUNT_NAME]);
	SendClientMessageEx(target, -1, "Administrador %s setou suas necessidades para 100.", AccountData[playerid][e_ACCOUNT_NAME]);

	CharacterData[target][e_CHARACTER_HUNGER] = 100;
	CharacterData[target][e_CHARACTER_THIRST] = 100;
	CharacterData[target][e_CHARACTER_SLEEP] = 100;
...
CMD:ir(playerid, params[])
{
	...
	if (target == INVALID_PLAYER_ID)
	{
	    ...
	    {
	    	SetPlayerPos(playerid, 1760.9249, -1877.5945, 13.6007);
	    	SetPlayerFacingAngle(playerid, 359.1800);
	    	SetPlayerInterior(playerid, 0);
	    	SetPlayerVirtualWorld(playerid, 0);
	    	SetCameraBehindPlayer(playerid);

	    	Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s foi para o spawn padrão do servidor.", AccountData[playerid][e_ACCOUNT_NAME]);
	    ...
	    else if(!strcmp(type, "entrada", true))
	    {
	    	...
	    	Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s foi para a entrada ID %i.", AccountData[playerid][e_ACCOUNT_NAME], target);
	    ...
	    else if (!strcmp(type, "interação", true) || !strcmp(type, "interacao", true) || !strcmp(type, "interaçao", true) || !strcmp(type, "interacão", true))
	    {
	    	...
	    	Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s foi para a interação ID %i.", AccountData[playerid][e_ACCOUNT_NAME], target);
	    ...
	    else if (!strcmp(type, "empresa", true))
	    {
	    	...
	    	Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s foi para a empresa ID %i.", AccountData[playerid][e_ACCOUNT_NAME], target);
	    ...
	    else if (!strcmp(type, "estoque", true))
	    {
	    	...
	    	Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s foi para o estoque ID %i.", AccountData[playerid][e_ACCOUNT_NAME], target);
	    ...
	    else if (!strcmp(type, "casa", true))
	    {
	    	...
	    	Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s foi para a casa ID %i.", AccountData[playerid][e_ACCOUNT_NAME], target);
	...
	else
	{
		...
		Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s foi até a posição de %s.", AccountData[playerid][e_ACCOUNT_NAME], Character_GetName(target));
...
CMD:noclip(playerid)
{
	...
	{	
		...
		Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s está agora em modo no-clip.", AccountData[playerid][e_ACCOUNT_NAME]);
...
CMD:ircoord(playerid, params[])
{
	...
	Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s teleportou-se para a posição %.01f, %.01f, %.01f", AccountData[playerid][e_ACCOUNT_NAME], x, y, z);
...
CMD:setmundo(playerid, params[])
{
	...
	Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s alterou o mundo virtual de %s para %i.", AccountData[playerid][e_ACCOUNT_NAME], Character_GetName(target, false), world);
...
CMD:setinterior(playerid, params[])
{
	...
	Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s alterou o interior de %s para %i.", AccountData[playerid][e_ACCOUNT_NAME], Character_GetName(target, false), interior);
...
CMD:reviver(playerid, params[])
{
	...
	Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s reviveu o personagem %s.", AccountData[playerid][e_ACCOUNT_NAME], Character_GetName(target, false));
...
CMD:setskin(playerid, params[])
{
	...
	Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s alterou a skin de %s para %i.", AccountData[playerid][e_ACCOUNT_NAME], Character_GetName(target, false), skin);
...
CMD:setvida(playerid, params[])
{
	...
	Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s alterou a vida de %s para %.0f.", AccountData[playerid][e_ACCOUNT_NAME], Character_GetName(target, false), health);
...
CMD:setcolete(playerid, params[])
{
	...
	Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s alterou o colete de %s para %.0f.", AccountData[playerid][e_ACCOUNT_NAME], Character_GetName(target, false), armour);
...
CMD:admhide(playerid)
{
	...
	if (AccountData[playerid][e_ACCOUNT_ADMIN_HIDE])
	{
		Admin_SendMessage(SENIOR_ADMIN, COLOR_YELLOW, "[PAINEL-ADMIN] %s está em modo administrador oculto.", AccountData[playerid][e_ACCOUNT_NAME]);
...
CMD:dararma(playerid, params[])
{
	...
	Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s deu a arma %s com %i munições para %s.", AccountData[playerid][e_ACCOUNT_NAME], ReturnWeaponName(weapon), ammo, Character_GetName(target, false));
...
CMD:reloadmaps(playerid)
{
	...
	Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s recarregou os mapas do servidor.", AccountData[playerid][e_ACCOUNT_NAME]);
...
CMD:dardinheiro(playerid, params[])
{
	...
	Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s deu %s para o personagem %s.", AccountData[playerid][e_ACCOUNT_NAME], FormatMoney(amount), Character_GetName(target, false));
...
CMD:setadmin(playerid, params[])
{
	...
	Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s definiu o nível admin de %s (%s) para %s.", AccountData[playerid][e_ACCOUNT_NAME], Character_GetName(target, false), AccountData[target][e_ACCOUNT_NAME], g_arrAdminLevelName[level]);
