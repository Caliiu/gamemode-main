...
CMD:criarinteracao(playerid, params[])
{
	...
	Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s criou uma interação de %s (%i)", AccountData[playerid][e_ACCOUNT_NAME], Interact_ReturnType(type), index);
	return true;
}
...
CMD:destruirinteracao(playerid, params[])
{
	...
	Admin_SendMessage(1, COLOR_LIGHTRED, "[PAINEL-ADMIN] %s deletou a interação %s (%i)", AccountData[playerid][e_ACCOUNT_NAME], Interact_ReturnType(InteractData[index][e_INTERACT_TYPE]), index);
