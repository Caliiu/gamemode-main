...
#define CHARACTER_TD_HOVER_COLOR 0xc1241fFF
...
hook OnGameModeInit()
{
	...
	TextDrawColor(s_tdBackground[1], -1306385665); // Antiga Azul 0x1474b8FF
...
{	
	...
	for (new i = 0; i < rows; i++)
	{
		...
		if (factionIndex == -1)
		{
			format (factionRank, sizeof factionRank, "~w~~h~Cidadao");
		}
		else
		{
	...
	if (rows < MAX_ACCOUNT_CHARACTER)
	{
		...
		PlayerTextDrawColor(playerid, s_ptdButton[playerid][rows][e_TD_BTN_BUTTON], 0xc1241fFF);
		...
		PlayerTextDrawColor(playerid, s_ptdButton[playerid][rows][e_TD_BTN_BACKGROUND], -1306385665); // Antiga Azul 0x1474b8FF
...
SelectCharacterIndex(playerid, index)
{
	...
	for (new i = 0; i < MAX_ACCOUNT_CHARACTER; i++)
	{
		...
		PlayerTextDrawColor(playerid, s_ptdButton[playerid][i][e_TD_BTN_BUTTON], ((index != -1 && i == index) ? 0xc1241fFF : 0x5f5f5fFF));
		PlayerTextDrawShow(playerid, s_ptdButton[playerid][i][e_TD_BTN_BUTTON]);
	}
